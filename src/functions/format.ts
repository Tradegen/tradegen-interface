import { Token, TokenAmount, Fraction, JSBI, Price } from '@ubeswap/sdk'
...
export function formatCurrencyAmount(amount: TokenAmount | undefined, sigFigs: number) {
  if (!amount) {
    return '-'
  }
  ...
  if (amount.divide(amount.quotient).lessThan(new Fraction(JSBI.BigInt(1), JSBI.BigInt(100000)))) {
    return '<0.00001'
  }
